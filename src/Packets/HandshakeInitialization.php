...
class HandshakeInitialization implements PacketInterface
{
    ...
    public static function unpack(Binary $binary): array
    {
        try {
            return Packet::parser(function (Binary $binary) {
                ...
                if ($filler !== 0) {
                    throw new PacketException("Error: Filler byte must be 0, found '$filler'");
                }
                // 6. 能力标识低 2 字节;
            ...
            throw new PacketException("Error: Failed to unpack handshake initialization packet [{$e->getMessage()}]", $e->getCode(), $e);
        }
    ...
    public static function pack(array $data): Binary
    {
        foreach (
            ...
            if (!isset($data[$field])) {
                throw new PacketException("Error: Missing required field '$field' for handshake packet.");
            }
        }
        ...
        try {
            return Packet::binary(function (Binary $binary) use ($data) {
                ...
                if (($authPluginDataLength = count($authPluginData)) < 8) {
                    throw new PacketException("Error: auth_plugin_data must be at least 8 bytes.");
                }
                foreach ($authPluginData as $byte) {
                    if (!is_int($byte) || $byte < 0 || $byte > 255) {
                        throw new PacketException("Error: auth_plugin_data must be an array of bytes.");
                    }
                }
            ...
            throw new PacketException("Error: Failed to pack handshake initialization packet [{$e->getMessage()}]", $e->getCode(), $e);
        }
    }
}
